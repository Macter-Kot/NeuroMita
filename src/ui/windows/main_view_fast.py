...
class ChatGUI(QMainWindow):
    ...
    def __init__(self, settings):
        ...
        # Инициализация атрибутов для анимации
        self._target_width = 0
        self._is_hiding = False
        self._start_width = 0

        # Создаём QTimeLine для анимации с высоким FPS
        self.settings_timeline = QTimeLine(300)  # Длительность 250 мс
        self.settings_timeline.setUpdateInterval(8)  # ~60 FPS (16 мс между кадрами)
        self.settings_timeline.setFrameRange(0, 1000)  # Диапазон для точности
        self.settings_timeline.setEasingCurve(QEasingCurve.Type.InOutQuad)

        # Подключаем обновление ширины на каждом кадре
        self.settings_timeline.frameChanged.connect(self._update_settings_width)

        # Подключаем завершение анимации
        self.settings_timeline.finished.connect(self._on_settings_animation_finished)
    ...
    def _update_settings_width(self, frame):
        """Обновляет ширину панели настроек на каждом кадре анимации"""
        progress = frame / 1000.0  # Прогресс от 0 до 1
        
        # Интерполяция между начальной и целевой шириной
        new_width = int(self._start_width + (self._target_width - self._start_width) * progress)
        self.settings_overlay.setMaximumWidth(new_width)
        self.settings_overlay.setFixedWidth(new_width)

    def _on_settings_animation_finished(self):
        """Вызывается при завершении анимации"""
        # Устанавливаем финальную ширину для точности
        self.settings_overlay.setMaximumWidth(self._target_width)
        self.settings_overlay.setFixedWidth(self._target_width)
        
        # Скрываем панель, если она закрывается
        if self._is_hiding:
    ...
    def show_settings_category(self, category):
        ...
        self._start_width = self.settings_overlay.width()
        self._target_width = 0 if is_hiding else self.SETTINGS_PANEL_WIDTH
