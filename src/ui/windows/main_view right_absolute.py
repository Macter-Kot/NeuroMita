...
class SettingsOverlay(QWidget):
    def __init__(self, parent):
        ...
        # Атрибут для поддержки полупрозрачности
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
        self.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self.setStyleSheet("""
            /* Сам оверлей с полупрозрачным фоном */
            QWidget#SettingsOverlay {
                background-color: transparent;
                /* ИЗМЕНЕНО: граница теперь слева */
                border-left: 2px solid #8a2be2;
            }
            /* И самое главное: делаем прозрачным фон у виджетов ВНУТРИ оверлея,
               чтобы его фон был виден */
        ...
        # QStackedWidget тоже нужно сделать прозрачным
        self.stack = QStackedWidget()
        lay.addWidget(self.stack)

        self.hide()

    # контейнеры добавляются один раз
    ...
    def show_category(self, container):
        ...
        self.raise_()
        # прокрутка в начало
        bar = self.stack.currentWidget().findChild(QScrollArea)
        if bar:
            bar.verticalScrollBar().setValue(0)
...
class ChatGUI(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # ИЗМЕНЕНО: Порядок добавления виджетов. Сначала чат, потом панель настроек.
    ...
    def _init_settings_containers(self):
        """Создаём контейнер для каждой категории настроек и сразу кладём в overlay-stack."""
        callbacks = {
        ...
        for key, fn in callbacks.items():
            ...
            # вызываем функцию корректно
            if isinstance(fn, types.MethodType) and fn.__self__ is self:
                fn(lay)                      # bound-method «self.func(layout)»
            else:
                fn(self, lay)                # external «setup(self, layout)»

            # разворачиваем CollapsibleSection и скрываем header
            i = 0
            while i < lay.count():
                item = lay.itemAt(i)
                w = item.widget()
                if isinstance(w, CollapsibleSection):
                    w.expand()               # разворачиваем
                    # переносим ВСЕ дочерние виджеты секции в layout контейнера
                    while w.content_layout.count():
                        ...
                        if sub_item.widget():
                            ...
                            i += 1
                    # удаляем секцию-обёртку
                    w.deleteLater()
                    lay.removeItem(item)     # сам item тоже убираем, без i++
                    continue                 # остаёмся на той же позиции
                i += 1

            lay.addStretch()
            self.settings_containers[key] = cont
            self.settings_overlay.add_container(cont)

    def show_settings_category(self, category):
        if self.current_settings_category == category and self.settings_overlay.isVisible():
        ...
        # ИЗМЕНЕНО: позиционируем overlay. Расчет теперь идет от правого края.
        overlay_width = self.settings_overlay.width()
        icon_bar_width = 50
        # x = (ширина окна) - (ширина панели иконок) - (ширина оверлея)
        new_x = self.width() - icon_bar_width - overlay_width
        self.settings_overlay.setGeometry(new_x, 0, overlay_width, self.height())
    ...
    def setup_settings_panel(self, main_layout):
        ...
        self.settings_overlay.setFixedWidth(400)
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'settings_overlay') and self.settings_overlay.isVisible():
            # ИЗМЕНЕНО: Расчет позиции оверлея при изменении размера окна
            overlay_width = self.settings_overlay.width()
            icon_bar_width = 50
            new_x = self.width() - icon_bar_width - overlay_width
            overlay_height = self.height()
            self.settings_overlay.setGeometry(new_x, 0, overlay_width, overlay_height)
    ...
    def prompt_for_code(self, code_future):
        self.event_bus.emit(Events.Telegram.REQUEST_TG_CODE, {'future': code_future})
    ...
    def prompt_for_password(self, password_future):
        self.event_bus.emit(Events.Telegram.REQUEST_TG_PASSWORD, {'future': password_future})
