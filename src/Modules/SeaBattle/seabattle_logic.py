...
# --- Движок игры (остается без изменений) ---
class SeaBattleEngine:
    ...
    def __init__(self, player_id: int = 0, llm_id: int = 1):
        self.player_id = player_id
        self.llm_id = llm_id
        self.reset()

    def reset(self):
        self.player_boards: Dict[int, List[List[int]]] = { self.player_id: self._create_empty_board(), self.llm_id: self._create_empty_board() }
        self.player_ships: Dict[int, List[Dict[str, Any]]] = { self.player_id: [], self.llm_id: [] }
        self.ships_to_place: Dict[int, List[int]] = { self.player_id: self.SHIP_CONFIG[:], self.llm_id: self.SHIP_CONFIG[:] }
    ...
    def place_ship(self, player_id, x, y, length, orientation):
        ...
        if not self.ships_to_place[self.player_id] and not self.ships_to_place[self.llm_id]: self.game_phase = "battle"
    ...
    def make_move(self, attacker_id: int, x: int, y: int):
        ...
        defender_id = self.llm_id if attacker_id == self.player_id else self.player_id
    ...
    # Остальные методы (place_all_llm_ships_randomly и т.д.) без изменений...
    def place_all_llm_ships_randomly(self):
        llm_id = self.llm_id
        for ship_len in self.ships_to_place[llm_id][:]:
            placed = False
            for _ in range(100):
                orient = random.choice(['h', 'v'])
                x, y = random.randint(0, 9), random.randint(0, 9)
                if self.place_ship(llm_id, x, y, ship_len, orient)[0]:
                    placed = True
                    break
            if not placed: return False, f"Не удалось разместить {ship_len}-палубный"
        return True, "Все корабли LLM размещены."


# --- Провайдер состояния (здесь основные изменения) ---
class GameStateProvider:
    ...
    """
    def __init__(self, player_id: int = 0, llm_id: int = 1):
        self.engine = SeaBattleEngine(player_id, llm_id)
        self.player_id = player_id
        self.llm_id = llm_id
    ...
    def get_full_state(self) -> Dict[str, Any]:
        """Возвращает полное состояние игры, включая тактическую аналитику и строки досок для LLM."""
        ...
        # Генерация представлений досок для LLM
        # "Моя доска" для LLM - это его собственная доска со всеми его кораблями
        llm_my_board_str = self._board_to_str(self.engine.player_boards[self.llm_id], 'my_full')
        # "Доска противника" для LLM - это то, как LLM видит доску игрока
        _, llm_opponent_view_raw = self._get_llm_views()
        llm_opponent_view_str = self._board_to_str(llm_opponent_view_raw, 'opponent_view')
        ...
        return {
            ...
            "llm_ships_to_place": self.engine.ships_to_place[self.llm_id][:],
            "player_ships_to_place": self.engine.ships_to_place[self.player_id][:],
            
            # Строки досок специально для промпта LLM
            "llm_my_board_str": llm_my_board_str,
            "llm_opponent_view_str": llm_opponent_view_str,
            
            # Тактическая аналитика для LLM
            "hunt_info": self._get_hunt_info_for_llm(),
            "shot_history_str": ", ".join(self._get_shot_history_for_llm()),
        }

    def _get_shot_history_for_llm(self) -> List[str]:
        """Возвращает список клеток, по которым LLM уже стрелял (по доске игрока)."""
        history = []
    ...
    def _get_hunt_info_for_llm(self) -> Dict[str, Any]:
        """Анализирует доску игрока и находит цели для LLM."""
        ...
        if not wounded_ships_coords: return {}
        ...
        for hit_list in wounded_ships_coords:
            if len(hit_list) == 1:
                ...
                coords_to_check = [c[0] if is_horizontal else c[1] for c in hit_list]
                min_c, max_c = min(coords_to_check), max(coords_to_check)
                if is_horizontal:
                    y = hit_list[0][1]
                    potential_targets.update([(min_c - 1, y), (max_c + 1, y)])
                else:
                    x = hit_list[0][0]
                    potential_targets.update([(x, min_c - 1), (x, max_c + 1)])
        
        shot_coords_tuples = {from_alg(c) for c in self._get_shot_history_for_llm()}
        valid_targets = [to_alg(x, y) for x, y in potential_targets if self.engine._is_valid_coord(x, y) and (x, y) not in shot_coords_tuples]
    ...
    def _get_player_views(self):
        ...
        opponent_board = self.engine.player_boards[self.llm_id]
        opponent_view = [[self.engine.VIEW_UNKNOWN] * 10 for _ in range(10)]
        for r, c in [(r,c) for r in range(10) for c in range(10)]:
    ...
    def _get_llm_views(self):
        """Возвращает доски с точки зрения LLM."""
        my_board = self.engine.player_boards[self.llm_id]
    ...
    def get_full_state(self) -> Dict[str, Any]:
        """Возвращает полное состояние игры, включая тактическую аналитику для LLM."""
        player_board, opponent_view = self._get_player_views()
        
        last_move_info = self.engine.last_move_result
        if last_move_info:
            last_move_info['coord_alg'] = to_alg(last_move_info['coords'][0], last_move_info['coords'][1])

        # Новая аналитика для LLM
        hunt_info = self._get_hunt_info_for_llm()
        shot_history = self._get_shot_history_for_llm()
        ...
        return {
            ...
            "player_board_raw": player_board,
            "opponent_view_raw": opponent_view,
            
            "llm_ships_to_place": self.engine.ships_to_place[self.llm_id][:],
            "player_ships_to_place": self.engine.ships_to_place[self.player_id][:],
            "last_move": last_move_info,
            
            # Новые данные для промпта
            "hunt_info": hunt_info,
            "shot_history_str": ", ".join(shot_history),
        }

    def _get_shot_history_for_llm(self) -> List[str]:
        """Возвращает список клеток, по которым LLM уже стрелял."""
        history = []
        player_board = self.engine.player_boards[self.player_id]
        for r in range(self.engine.BOARD_SIZE):
            for c in range(self.engine.BOARD_SIZE):
    ...
    def _get_hunt_info_for_llm(self) -> Dict[str, Any]:
        ...
        shot_coords = {from_alg(c) for c in self._get_shot_history_for_llm()}
        valid_targets = []
